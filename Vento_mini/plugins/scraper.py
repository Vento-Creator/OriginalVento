...
GIRL_NAMES = {
    ...
    "hulkar", "shoira", "oisha", "oysha", "oygul", "oynisa",
...
_scraper_states = {}       # uid -> "waiting_for_scrape_target" / "waiting_msg_count_{uid}"
_scraper_targets = {}      # uid -> {"target": chat_id}
_stop_flags = {}           # stop_key -> bool
_active_processes = {}     # stop_key -> {"user_id": int, "target": int, "status_msg": Message}
...
@Client.on_callback_query(filters.regex("^(scraper_menu|menu_scraper)$"))
async def scraper_start(client: Client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    if not await check_account_guard(callback_query):
        return

    try:
        del_msg = await callback_query.message.reply_text("⏳")
        await del_msg.delete()
    except Exception:
        pass

    _scraper_states[user_id] = "waiting_for_scrape_target"
...
async def scraper_link_handler(client: Client, message: Message):
    ...
    state = _scraper_states.get(user_id)
    ...
    try:
        ...
        _scraper_states.pop(user_id, None)
        err_str = str(e)
        if "AUTH_KEY_UNREGISTERED" in err_str or "SESSION" in err_str.upper() or "🔑" in err_str:
            await message.reply_text(
                f"{e}",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔗 Akkauntni qayta ulash", callback_data="menu_account_back")],
    ...
    _scraper_targets[user_id] = {"target": chat.id}
    _scraper_states.pop(user_id, None)
...
async def execute_fast_scrape(user_id: int, target: int, status_msg: Message, client: Client):
    """Tez (odatiy) scrape — a'zolar ro'yxati orqali."""
    stop_key = f"scraper_{user_id}_{int(time.time())}"
    _stop_flags[stop_key] = False
    ...
    try:
        ...
        _active_processes.pop(stop_key, None)
...
async def execute_msg_scrape(user_id: int, target: int, msg_limit: int, status_msg: Message, client: Client):
    """Habarlar orqali scrape — chat tarixidan faol userlarni yig'ish."""
    stop_key = f"scraper_{user_id}_{int(time.time())}"
    _stop_flags[stop_key] = False
...
async def scrape_fast_callback(client: Client, callback_query: CallbackQuery):
    ...
    if len(_active_processes) >= MAX_CONCURRENT_SCRAPERS:
        await callback_query.answer("⚠️ Serverda hozircha ko'p ishlayapti! Iltimos, keyinroq urinib ko'ring.", show_alert=True)
        return

    user_active = sum(1 for p in _active_processes.values() if p["user_id"] == user_id)
    ...
    _active_processes[stop_key] = {"user_id": user_id, "target": target, "status_msg": msg}

    await callback_query.answer()
    await execute_fast_scrape(user_id, target, msg, client)
...
async def scrape_msg_callback(client: Client, callback_query: CallbackQuery):
    ...
    _scraper_states[user_id] = f"waiting_msg_count_{user_id}"
    await callback_query.message.edit_text(
...
async def scrape_msg_count_handler(client: Client, message: Message):
    ...
    if state != f"waiting_msg_count_{user_id}":
        raise ContinuePropagation
    ...
    _active_processes[stop_key] = {"user_id": user_id, "target": target, "status_msg": status_msg}

    await execute_msg_scrape(user_id, target, msg_limit, status_msg, client)
...
async def stop_scraper_callback(client: Client, cq: CallbackQuery):
    ...
    process = _active_processes.get(stop_key)
